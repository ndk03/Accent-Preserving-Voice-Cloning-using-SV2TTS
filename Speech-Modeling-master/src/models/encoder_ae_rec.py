...
class ACCENT_ENCODER_AE_REC(nn.Module):
    def __init__(self,
                 input_shape=(160, 40),
                 load_model=False,
                 epoch=0,
                 ...
                 loss_=None):
        super(ACCENT_ENCODER_AE_REC, self).__init__()
        ...
        self.linear2 = nn.Linear(2 * self.config_yml['MEL_CHANNELS'],
                                 self.config_yml['MEL_CHANNELS'])
        self.lstme = nn.LSTM(self.config_yml['MEL_CHANNELS'],
                             self.config_yml['HIDDEN_SIZE'],
                             bidirectional=True,
                             batch_first=True,
                             num_layers=3)
        self.lstmd = nn.LSTM(2 * self.config_yml['HIDDEN_SIZE'],
                             self.config_yml['MEL_CHANNELS'],
                             bidirectional=True,
                             batch_first=True,
                             num_layers=3)
    ...
    def forward(self, frames):
        embs_, (_, _) = self.lstme(frames)
        embs = embs_.mean(dim=1)
        embs = self.linear1(embs)
        x, (_, _) = self.lstmd(embs_)
        x = self.linear2(x)
    ...
    def embed(self, aud):
        ...
        frames = torch.Tensor(frames).view(
            ...
            self.config_yml['MEL_CHANNELS'],
            self.config_yml['SLIDING_WIN_SIZE'],
        ).transpose(1, 2).to(device=self.device)
...
if __name__ == "__main__":
    ...
    encoder = ACCENT_ENCODER_AE_REC(dataset_train=args.dataset_train,
