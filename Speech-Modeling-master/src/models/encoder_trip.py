...
class ACCENT_ENCODER_TRIPLET(nn.Module):
    def __init__(self,
                 input_shape=(160, 40),
                 load_model=False,
                 epoch=0,
                 ...
                 loss_=None):
        super(ACCENT_ENCODER_TRIPLET, self).__init__()
    ...
    def triple_loss_fn(self, loss_, embeds):
        ...
        for i, uttrs in enumerate(embeds):

            lang = i//self.config_yml['UTTR_COUNT']
            
            lang_mean = torch.mean(embeds3d[lang], dim=1)
            triple_loss_ = nn.TripletMarginLoss()
            loss = [triple_loss_(uttrs.reshape(-1, 1), centroids[lang].reshape(-1, 1), centroids[i].reshape(-1,1)) for i in range(lang_count) if(i!=lang)]
            losses.append(torch.sum(torch.stack(loss)))
        sim_matrix = embeds*0
        return torch.sum(torch.stack(losses)), sim_matrix
    ...
    def train_loop(self,
        ...
        for epoch in range(self.epoch, 20000):
            for i, (data, labels) in enumerate(train_iterator):
                ...
                self.loss, sim_matrix = self.triple_loss_fn(loss_, embeds)
...
if __name__ == "__main__":
    ...
    encoder = ACCENT_ENCODER_TRIPLET(dataset_train=args.dataset_train,
